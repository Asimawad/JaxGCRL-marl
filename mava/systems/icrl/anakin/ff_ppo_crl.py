...
def get_learner_fn(
    ...
    # When use_gae is True, advantages are computed via GAE bootstrapping over
    # V(s,g) = E_pi[Q(s,.,g)] from the contrastive critic, rather than the
    # single-step Q(s,a,g) - V(s,g) we use by default. No explicit reward —
    # the contrastive V already encodes future-goal-reaching.
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def compute_crl_gae_advantages(traj_batch, params):
            """GAE-bootstrapped advantages using the contrastive V as the value
            function. No explicit reward — the contrastive V already encodes
            log P(reach g from s) so the TD residual

                delta_t = gamma * V_{t+1} * (1 - done_t) - V_t

            measures one-step *progress toward the goal*. GAE then smooths this
            across multiple future steps, in the same way standard PPO smooths
            ...
            actor sees in its loss).

            Returns advantages of shape (T, E).
            """
            obs_achieved = jnp.concatenate(
                [traj_batch.observation, traj_batch.achieved_goal], axis=-1
            )  # [T, E, S]
            T_, E_ = obs_achieved.shape[:2]
            S_ = obs_achieved.shape[-1]
            # Per-action SA encoder, batched on a new axis.
            obs_tiled = jnp.broadcast_to(
                obs_achieved[..., None, :], (T_, E_, action_dim, S_)
            )  # [T, E, A, S]
            actions_tiled = jnp.broadcast_to(
                jnp.eye(action_dim, dtype=obs_achieved.dtype)[None, None, :, :],
                (T_, E_, action_dim, action_dim),
            )  # [T, E, A, A]
            sa_repr = sa_encoder_apply_fn(
                params.sa_encoder, obs_tiled, actions_tiled
            )  # [T, E, A, R]
            g_repr = g_encoder_apply_fn(
                params.goal_encoder, traj_batch.ultimate_goal
            )  # [T, E, R]
            q_values = energy_fn(energy_fn_name, sa_repr, g_repr[..., None, :])  # [T, E, A]

            actor_input = jnp.concatenate(
                [traj_batch.observation, traj_batch.achieved_goal, traj_batch.ultimate_goal],
                axis=-1,
            )
            actor_obs = Observation(
                agents_view=actor_input, action_mask=traj_batch.avail_actions
            )
            pi = actor_apply_fn(params.actor, actor_obs)
            action_probs = jax.nn.softmax(pi.distribution.logits)  # [T, E, A]
            values = jnp.sum(action_probs * q_values, axis=-1)  # [T, E]
