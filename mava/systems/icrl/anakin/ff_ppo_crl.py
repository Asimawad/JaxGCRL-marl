...
def get_learner_fn(
    ...
    # Decoupled minibatch sizes. The InfoNCE critic loss builds a [B, B]
    # logits matrix and is memory-bound at B > ~1024; the PPO actor loss has
    # no such constraint and benefits from much larger batches (fewer SGD
    # steps per update => less per-iter scan overhead). actor_batch_size
    # defaults to batch_size for backward compatibility.
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _update_epoch(carry, _):
            """One PPO epoch: independent actor and critic passes over the same data.

            Actor and critic params are disjoint and the per-step advantages
            are frozen before this loop, so doing two separate scans (instead
            of an interleaved actor+critic minibatch) is mathematically
            equivalent up to optimizer-step ordering — and lets each side use
            its own minibatch size: large for the actor (no [B, B] matrix),
            small for the critic (InfoNCE logits matrix is O(B^2) memory).
            """
            params, opt_states, key, flat_transitions = carry
            n = flat_transitions.observation.shape[0]

            # ---- Actor pass ----
            key, perm_key_a = jax.random.split(key)
            perm_a = jax.random.permutation(perm_key_a, n)
            shuffled_a = jax.tree_util.tree_map(lambda x: x[perm_a], flat_transitions)
            batched_a = jax.tree_util.tree_map(
                lambda x: jnp.reshape(x, (-1, actor_batch_size) + x.shape[1:]),
                shuffled_a,
            )
            (new_actor, new_actor_opt, key), actor_metrics = jax.lax.scan(
                _actor_update_minibatch,
                (params.actor, opt_states.actor, key),
                batched_a,
                unroll=scan_unroll,
            )

            # ---- Critic pass ----
            key, perm_key_c = jax.random.split(key)
            perm_c = jax.random.permutation(perm_key_c, n)
            shuffled_c = jax.tree_util.tree_map(lambda x: x[perm_c], flat_transitions)
            batched_c = jax.tree_util.tree_map(
            ...
            (new_critic, new_critic_opt, key), critic_metrics = jax.lax.scan(
                _critic_update_minibatch,
                (critic_init, opt_states.critic, key),
                batched_c,
                unroll=scan_unroll,
            )
            ...
            # Mean-reduce per-minibatch metrics so actor and critic series
            # have matching shapes for the outer-scan stack + later logging.
            metrics = {
                **jax.tree_util.tree_map(jnp.mean, actor_metrics),
                **jax.tree_util.tree_map(jnp.mean, critic_metrics),
            }
            return (new_params, new_opt_states, key, flat_transitions), metrics
...
def learner_setup(env: MarlEnv, key: chex.PRNGKey, config: DictConfig) -> Tuple:
    ...
    critic_minibatches = samples_per_update // critic_batch_size
    actor_total_grad_steps = config.system.num_updates * config.system.ppo_epochs * actor_minibatches
    critic_total_grad_steps = config.system.num_updates * config.system.ppo_epochs * critic_minibatches
