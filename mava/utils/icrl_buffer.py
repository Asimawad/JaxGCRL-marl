...
RelabledObservation = namedtuple('RelabledObservation', ['observation', 'goal', 'action', 'avail_actions'])
...
class TrajectoryUniformSamplingQueue():
    ...
    def flatten_crl_fn(buffer_config, transition, sample_key):
        ...
        goal = jnp.take(transition.extras["achieved_goal"], goal_index[:-1], axis=0) #the last goal_index cannot be considered as there is no future.  

        return RelabledObservation(
            observation=transition.observation[:-1],
            goal=goal,
            action=transition.action[:-1],
            avail_actions=transition.avail_actions[:-1],
        )
