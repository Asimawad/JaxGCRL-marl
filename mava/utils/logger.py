...
class WandbLogger(BaseLogger):
    def __init__(
        ...
        if run_id is not None:
            self.logger = wandb.init(
                id=run_id, resume="must", project=project, entity=entity
            )
        else:
            run_tags = list(tags) if tags else []
            if "AICHOR_EXPERIMENT_ID" in os.environ:
                run_tags.append(os.environ.get("AICHOR_USER_NAME", ""))
                run_tags.append(os.environ.get("VCS_SHA_SHORT", ""))
                run_tags = [t for t in run_tags if t]
    ...
    def log_stat(self, key: str, value: float, step: int, eval_step: int, event: LogEvent) -> None:
        ...
        value = value.item() if isinstance(value, (jax.Array, np.ndarray)) else value
        self.logger.log({f"{event.value}/{key}": value}, step=step)
