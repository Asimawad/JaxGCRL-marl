...
class RecordEpisodeMetrics(Wrapper):
    ...
    def reset(self, keys: chex.PRNGKey) -> Tuple[RecordEpisodeMetricsState, TimeStep]:
        """Reset the environment."""
        wrapped_state, timestep = self._env.reset(keys)
        if keys.ndim == 1:
            num_envs = ()
        else:
        ...
        episode_metrics_dict = {
            "episode_return": jnp.zeros(num_envs, dtype=float),
            "episode_length": jnp.zeros(num_envs, dtype=int),
            "is_terminal_step": jnp.zeros(num_envs, dtype=bool),
        }
