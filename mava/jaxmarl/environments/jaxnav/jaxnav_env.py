...
class JaxNav(MultiAgentEnv):
    ...
    def step_env(
        ...
        update_state_valid = agent_states.done | agent_states.move_term
        ...
        if self.evaporating:
            ...
            dones = goal_reached | time_up | agent_states.done  # OR operation over agent status
            ep_done = jnp.all(dones) 
        else:
            goal_reached = goal_reached | old_goal_reached
            collisions = collisions | old_move_term
            ep_done = jnp.all(goal_reached | time_up)
    ...
    def compute_reward(
        ...
        rc = jnp.zeros_like(rg)  # collision reward disabled
