#!/usr/bin/env bash
# CPPO DECOUPLED + GAE (smooth_adv) — current best on smacv2_5_units.
# Same as run_cppo_decoupled.sh + GAE smoothing of the contrastive
# single-step advantage Q(s,a) - V(s) over time.
# Confirmed at 80M / 512 envs: peaks ~79-80% win rate (above non-GAE 76.51%).

set -euo pipefail
source .venv/bin/activate

export WANDB_API_KEY=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
WANDB_GROUP="cppo_decoupled_gae-smacv2_5_units"
...
python -u -m mava.systems.icrl.anakin.ff_ppo_crl -m system.seed=0,1,2,3,4 \
    ${COMMON_ARGS} \
    logger.loggers.wandb.run_name=cppo_decoupled_gae-smacv2-5units-V1 \
    logger.loggers.wandb.tags=["cppo_decoupled_gae","ff_ppo_crl","smacv2-5units","gae","smooth_adv"]
