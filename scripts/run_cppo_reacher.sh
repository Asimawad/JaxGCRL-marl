#!/bin/bash
# CPPO on JaxGCRL Reacher — same recipe that hit 0.623 peak on Ant.
# Paper CRL @ 50M: ~1.00 (Reacher is solved by all baselines).
# Sanity-check env. Should hit ≥0.95 in <10M steps.

: "${WANDB_API_KEY:?Set WANDB_API_KEY in your environment before running}"
source .venv/bin/activate

DATE=$(date +%Y-%m-%d)
GROUP="cppo-reacher-${DATE}"
...
for SEED in $SEEDS; do
    EXP_NAME="cppo-reacher-seed${SEED}-${DATE}"
