#!/usr/bin/env bash
# CPPO BASELINE (pre-optimization) — for paper SPS comparison and as a control.
# Same code as ff_ppo_crl.py before the v1-v4 optimizations.
# Hparams: tuned Run-9 config from run.sh (win_rate=76.07%).

set -euo pipefail
source .venv/bin/activate

export WANDB_API_KEY=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
WANDB_GROUP="cppo_baseline-smacv2_5_units"
...
python -u -m mava.systems.icrl.anakin.ff_ppo_crl_baseline -m system.seed=0,1,2,3,4 \
    ${COMMON_ARGS} \
    logger.loggers.wandb.run_name=cppo_baseline-smacv2-5units-V1 \
    logger.loggers.wandb.tags=["cppo_baseline","ff_ppo_crl","smacv2-5units"]
