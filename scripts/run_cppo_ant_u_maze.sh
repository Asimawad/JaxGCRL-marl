#!/bin/bash
# CPPO on JaxGCRL Ant U-Maze — same recipe that hit 0.623 peak on Ant.
# Paper CRL @ 50M: ~0.50.
# Same 8-dim action as ant, but adds exploration challenge from the maze geometry.

: "${WANDB_API_KEY:?Set WANDB_API_KEY in your environment before running}"
source .venv/bin/activate

DATE=$(date +%Y-%m-%d)
GROUP="cppo-ant_u_maze-${DATE}"
...
for SEED in $SEEDS; do
    EXP_NAME="cppo-ant_u_maze-seed${SEED}-${DATE}"
