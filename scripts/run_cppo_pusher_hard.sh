#!/bin/bash
# CPPO on JaxGCRL Pusher Hard — same recipe that hit 0.623 peak on Ant.
# Paper CRL @ 50M: ~0.50.
# Note: requires --backend=mjx (default backend hangs in JIT for this env).

: "${WANDB_API_KEY:?Set WANDB_API_KEY in your environment before running}"
source .venv/bin/activate

DATE=$(date +%Y-%m-%d)
GROUP="cppo-pusher_hard-${DATE}"
...
for SEED in $SEEDS; do
    EXP_NAME="cppo-pusher_hard-seed${SEED}-${DATE}"
    echo "=== seed=$SEED  steps=$STEPS  num_envs=$NUM_ENVS  backend=mjx ==="
